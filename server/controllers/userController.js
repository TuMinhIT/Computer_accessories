...
export const login = async (req, res) => {
    try {
        ...
        if (!user.isActive) return res.status(403).json({ message: 'Please activate your account via email' });
...
export const createStaff = async (req, res) => {
    try {
        ...
        await sendEmail(email, 'Account Activation', `Click this link to activate: ${link}`);
